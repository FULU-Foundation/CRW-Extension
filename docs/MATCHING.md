...
- `enableMatchAcrossTLDs` (`boolean`, default: `true`): allows cross-TLD alias matches for the same domain label when there is international suffix evidence (for example a compound suffix like `dyson.co.uk` vs `dyson.com.au`, or a ccTLD like `ford.ca` vs `ford.com`). Government suffixes (`gov` and `gov.*`, such as `gov.uk`) are always excluded from this heuristic. Produces a `subdomain` match with a `cross_tld_alias` reason.
